...
namespace yazi
{
    namespace object
    {
        template <typename T, int ChunksPerBlock> // ChunksPerBlock代表每个区块的大小
        ...
        public:
            ...
            virtual T* allocate(){
                if(m_head==nullptr){
                    if(sizeof(T)<sizeof(T *))//必须保证chunk大于等于一个指针的大小
                    {
                        std::cerr<<"object size less than point size."<<std::endl;
                        throw std::bad_alloc();
                    }
